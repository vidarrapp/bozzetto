...
- **Turntable** — `←` / `→` spin the model about its own centre (never the stroke pivot, so it stays a turntable wherever you've been working), with a much wider acceleration range: a slow wheel creeps at well under a degree a tick, a fast one covers ~20°, and a quick spin coasts briefly before settling.
