...
- **Wheel-friendly keys** — made for mapping to controller wheels (TourBox and friends): `[` / `]` step brush size, `Shift+[` / `]` step strength (with a brief on-screen ring so you see the size without hover), and `←` / `→` turn the model one degree per tick.
