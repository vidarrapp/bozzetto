...
- **Masking** — paint masks with `Ctrl`; masked areas darken on the model (ZBrush-style, in every material mode) and resist every brush. A `Ctrl`-click on empty space inverts the whole mask and a `Ctrl`-drag on empty space clears it (SculptGL parity).
...
- **Pen pressure** — Apple Pencil and other styluses drive brush strength fully through the stroke while brush size stays constant; mouse and plain touch are unaffected.
...
- **Sculpt rendering defaults, tuned for framerate** — flat shading, a single key light, shadows off, no depth of field, no HDRI sampling, the stage (floor/pedestal) hidden so the sculpt's underside stays visible (`G` brings it back), and GTAO replaced by a lightweight depth-based SSAO that keeps creases readable without gridding on flat-shaded facets. Everything returns to the saved look when leaving sculpt mode.
- **Touch toolbar** — a bottom bar for keyboard-less iPads: a Negative button in the left corner (hold to carve, like holding `Alt`, sculpting with a second finger — or double-tap to lock carving on, single tap to unlock) and the nine brushes synced with the hotkeys, each with an icon ([Uicons by Flaticon](https://www.flaticon.com/uicons)) and its hotkey digit as a corner badge.
- **Shading from the Render panel** — the regular Render panel drives sculpt shading too: material mode, albedo, roughness, matcaps, smooth/flat, wireframe, plus a new master **Shadows** toggle (also `Shift+S` while sculpting). Depth-of-field controls hide during sculpt, staying a view-mode concern.
...
- **Scene keys** — `F` frames the model while the orbit pivot automatically follows your strokes; `L` + drag rotates the light rig, and the whole rig follows your orbiting like the model is turning in your hand — the underside is lit when you look at it from below. The `H` hotkey guide switches between the viewer and sculpt tables automatically.
- **Sculpt palette** — a docked Sculpt panel (right edge, below Render): per-brush pressure dynamics (size/strength toggles with response curves), the active brush's feel sliders (Move falloff, clay-strip shape), mask tools (darken slider, blur/sharpen/invert/clear, and **Extract** with a thickness slider — the masked region becomes a new object), dynamic-topology detail, **voxel remesh**, and the sculpt SSAO's strength/radius.
- **Four docked panels, two per edge** — **File** over **Scene** on the left, **Render** over **Sculpt** on the right, each a labelled tab you pull out. Only one panel per edge is open at a time (they'd cover each other's tab), but a left and a right panel happily share the screen. **Scene** is the outliner: the scene's objects, click to select, plus-menu to add Sphere/Cube/Cylinder/Torus — sculpting is multi-object, so extractions and added shapes render, sculpt, persist and undo alongside the original. **Render** is the old Settings panel, renamed for what it actually does.
...
- **Hide the interface** — `Tab` clears every panel, toolbar and overlay, leaving just the model on an empty page for focused work with a keyboard or a TourBox (there's a hide button in the toolbar too, since iPads have no `Tab` key). Getting back is deliberately generous: `Tab` again, `Esc`, or the small eye that stays in the top-left corner. It never persists across a reload, so reloading is always a way out; and if you tap around a screen that isn't responding, the eye lights up and the hint returns — keep tapping and the interface simply comes back. The explanation fades away as you learn it: the first couple of times it spells the way out, after that it whispers, then it goes quiet.
- **Save, open, export** — the File panel keeps your work beyond the browser: **Save file** writes the full scene (every object, the whole subdivision stack, masks, symmetry) as a compact `.bozz` file, **Open file** loads one back, **Export OBJ** writes the visible scene for other apps. All of it stays on your device — the sculpt page is public, and nothing uploads.
