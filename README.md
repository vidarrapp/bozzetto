...
- **Masking gestures** — besides painting masks with `Ctrl`, a `Ctrl`-click on empty space inverts the whole mask and a `Ctrl`-drag on empty space clears it (SculptGL parity). Masked areas resist every brush.
...
- **Wheel-friendly keys** — made for mapping to controller wheels (TourBox and friends): `[` / `]` step brush size, the two keys below them (`;` / `'`) step strength (with a brief on-screen ring and strength line so you see both without hover), and `←` / `→` turn the model one degree per tick.
