...
- **Sculpt rendering defaults, tuned for framerate** — flat shading, a single key light, shadows off, no depth of field, no HDRI sampling, and GTAO replaced by a lightweight depth-based SSAO that keeps creases readable without gridding on flat-shaded facets. Everything returns to the saved look when leaving sculpt mode.
