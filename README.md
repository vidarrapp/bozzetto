...
- **Scene outliner** — a left-docked panel (closed by default) listing the scene's objects: click to select, plus-menu to add Sphere/Cube/Cylinder/Torus. Sculpting is multi-object now — extractions and added shapes render, sculpt, persist, and undo alongside the original. Save/load/export will live here later.
- **Side sliders** — two minimal vertical sliders on the left edge, Procreate-style: brush size (log-scaled) and strength. Dragging shows a centered preview ring so you see the size land; the nubs track every other way of changing the brush (keys, `B`/`S` drags, tool switches).
