...
- **Boot splash** — entering sculpt mode plays the handwritten Bozzetto logo writing itself (2.5 s) over the loading overlay, in an ink or paper variant to match the theme — the videos' backgrounds are the design system's warm ink and paper, so they melt into the page. A tap skips it, it can never hold the boot past a few seconds even if the video fails to load, and `prefers-reduced-motion` keeps the plain loading text.
