...
- **Detail** — the default sphere is ~50k triangles with a multiresolution stack: `D` / `Shift+D` step levels, `Ctrl+D` subdivides (capped at ~4M triangles). Dynamic topology runs under the hood and gets a UI later.
...
- **Touch toolbar** — a bottom bar for keyboard-less iPads: a hold-to-carve Negative button in the left corner (strokes carve while it's held, like holding `Alt`; sculpt with a second finger) and the nine brushes synced with the hotkeys, each with an icon ([Uicons by Flaticon](https://www.flaticon.com/uicons), solid straight) and its hotkey digit as a corner badge.
