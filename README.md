...
- **iPad: land the Pencil before your hand** — iPadOS delivers only one kind of touch to a web page at a time. A finger anywhere on the screen makes an Apple Pencil invisible to the browser: no event is generated at all, so no web app can work around it (we spent four rounds proving that, see `docs/sculpt-mode-implementation.md` §6.6e). The lock goes to whichever touch lands *first*, though, so it cuts both ways — put the Pencil down before resting your hand and the hand is the thing that gets ignored, which is the palm rejection you wanted anyway.
...
- **Continue where you left off** — leaving sculpt mode for the gallery snapshots the viewport, and your unfinished sculpt appears there as an "In progress" card. Clicking it drops you straight back in with the scene restored.
...
- **Scene keys** — `F` frames the model (and re-centres the orbit on it); `L` + drag rotates the light rig, and the whole rig follows your orbiting like the model is turning in your hand — the underside is lit when you look at it from below. The `H` hotkey guide switches between the viewer and sculpt tables automatically.
