...
- **Sculpting** — drag on the mesh to sculpt, drag off it to orbit; hold `Alt` to carve (negative), `Shift` to smooth, `Ctrl` to mask, `Ctrl+Alt` to unmask. Six brushes on `1`–`6` (Crease, Move, Standard clay, Inflate, Pinch, Flatten), `X` toggles symmetry, `Ctrl+Z` / `Ctrl+Shift+Z` undo and redo.
...
- **Touch toolbar** — a bottom bar for keyboard-less iPads: a Negative toggle in the left corner (strokes carve while lit; `Alt` still inverts relative to it) and the six brushes as numbered buttons synced with the hotkeys. Icons come later.
