...
- **Sculpt palette** — a docked Sculpt panel (right edge, below Settings): per-brush pressure dynamics (size/strength toggles with response curves), the active brush's feel sliders (Move falloff, clay-strip shape), mask tools (darken slider, blur/sharpen/invert/clear, and **Extract** with a thickness slider — the masked region becomes a new object), dynamic-topology detail, **voxel remesh**, and the sculpt SSAO's strength/radius.
- **Scene outliner** — a left-docked panel (closed by default) listing the scene's objects: click to select, plus-menu to add Sphere/Cube/Cylinder/Torus. Sculpting is multi-object now — extractions and added shapes render, sculpt, persist, and undo alongside the original. The file row beneath holds Save/Open/Export.
- **Timelapse capture** — sculpting records itself, Procreate-style: after each stroke (in idle time, never during one) the scene is snapshotted, quantized and gzipped to a timelapse frame by a worker, and stored in the browser alongside the autosave — captured frames survive reloads. The Scene panel's Capture section shows a live frame/size readout with a record toggle and a Clear button; capture stands down by itself past a ~500 MB frame budget.
- **Save, open, export** — the Scene panel's File section keeps your work beyond the browser: **Save file** writes the full scene (every object, the whole subdivision stack, masks, symmetry) as a compact `.bozz` file, **Open file** loads one back, **Export OBJ** writes the visible scene for other apps. All of it stays on your device — the sculpt page is public, and nothing uploads. The Scene panel is home base for your work: objects, files, and the recorder all live there.
- **Publish to the gallery (admin)** — signed in through Cloudflare Access, two extra forms appear in the Scene panel: publish the captured **timelapse** as a real gallery project (each frame a normal Bozzetto GLB frame), or publish the current scene as a single-frame **model** project. Guests never see these; the endpoints are Access-gated regardless.
