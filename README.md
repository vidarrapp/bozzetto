...
- **Why some rake stencils bite and others don't** — worth knowing before you wonder if the brush is broken. The stencil is mapped onto the square inscribed in the brush disc, so a tine's width on the model is about 0.7× its width in the image. Measured across the set, that lands between 10.7% of the brush diameter and 1.4%, and below roughly 6% a tine is thinner than the gap between vertices on an unsubdivided sphere: it falls between them, and the stroke comes out looking like plain clay. The picker is ordered broadest-first for that reason and defaults to the broadest, and the fine ones are kept because they do come alive on a subdivided mesh or at a much larger brush — not because they work everywhere. Nine stencils ship; four are ZBrush's stock rake alphas, standing in until hand-authored ones land.
