...
- **Sculpting** — drag on the mesh to sculpt, drag off it to orbit; hold `Alt` to carve (negative), `Shift` to smooth, `Ctrl` to mask, `Ctrl+Alt` to unmask. Nine brushes on `1`–`9` (Crease, Move, Standard clay, Inflate, Pinch, Flatten, Smooth, Drag, Polish), `X` toggles symmetry, `Ctrl+Z` / `Ctrl+Shift+Z` undo and redo — history is 64 deep and fuzz-tested.
- **ZBrush-informed brushes** — the Standard brush lays down clay-strip-style ribbon layers (flat-topped falloff, thicker build plane); the Move brush grabs volumetrically, so you can pull a silhouette starting from just outside the outline (with an aim ring off-model), with a softer, broader falloff. **Polish** replaces Twist on `9` (hPolish-style): a strong flat-topped planarizer that clips its effect to geometry near the sampled plane, so it smooths a surface right up to an edge and leaves the edge — or a corner — crisp where Flatten would melt it; `Alt` fills hollows up to the plane instead of shaving proud material down.
...
- **Object transforms** — a gizmo at last: the toolbar's Transform button shows the unified manipulator (translate arrows with view-plane handles, the three rotate rings plus Maya's screen-space outer ring, scale cubes inside), while `E`, `R` and `T` expose one mode each — move, rotate, scale — and `Q` returns to sculpting, as does picking any brush. Tap another object and the gizmo hops to it. Each completed drag is one undo entry, the camera is parked while a handle is held, and a moved object persists like any other edit. The unified gizmo is trimmed of the handles that stacked on top of each other: the view-plane translate centre and the two-axis translate/scale planes leave (they remain in the single modes, where nothing overlaps them), and the uniform-scale cube in the middle grows into an easy target. (The vendored SculptGL transform drew through the GL pipeline Bozzetto cut, so the gizmo is three.js's TransformControls driving the same matrices.)
...
- **Sculpt palette** — a docked Sculpt panel (right edge, below Render): brush size and strength sliders with their per-brush pressure dynamics (toggles and response curves) underneath, the active brush's feel sliders (Move falloff, clay-strip shape), mask tools (darken slider, blur/sharpen/invert/clear, and **Extract** with a thickness slider — the masked region becomes a new object), a Topology section with the multiresolution level controls and the dynamic-topology switch, and a **Remesh** section with a numbered resolution slider and **voxel remesh**. Cavity/SSAO lives in the Render panel with the rest of the shading.
- **Four docked panels, two per edge** — **File** over **Scene** on the left, **Render** over **Sculpt** on the right, each a labelled tab you pull out. Only one panel per edge is open at a time (they'd cover each other's tab), but a left and a right panel happily share the screen. **Scene** is the outliner: each object's row carries an **eye** (visibility) and a **padlock** (edit lock) before the name, and the selected row a **trash can** after it; click selects, **double-click renames** in place. A hidden object drops out of the viewport and out of picking; a locked one stays visible and selectable, but strokes and the transform gizmo refuse it and orbit instead. Both flags ride the autosave and `.bozz` files. Adding goes through a wide **Add to scene** button under the list (Sphere/Cube/Cylinder/Torus); deleting asks a plain `Delete "Sphere 2"?` — `Ctrl+Z` brings the object back, like any other edit. Sculpting is multi-object, so extractions and added shapes render, sculpt, persist and undo alongside the original. Under the object list sits the material dropdown for the selection, and **New material** asks for a name. **Render** is the old Settings panel, renamed for what it actually does.
