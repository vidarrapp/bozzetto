...
- WS6 Layout (owner round, shipped). Four docked panels, two per edge: File over Scene on the left mirroring Render over Sculpt on the right, on a shared `SidePanel` base with a side-scoped `bozzetto:panel-open` protocol. The settings panel is renamed **Render** (tab always; header during sculpt, which also resolves a "Sculpt above Sculpt" collision from the synthetic manifest title). `Tab` hides all chrome for a minimal working screen, claimed in InputShell's capture phase so the viewer's `Tab` is untouched. Hotkey table additions: `Tab` = close open panels, then hide the interface (staged); `Esc` = show it; `Ctrl+C` / `Ctrl+I` / `Ctrl+H` / `Ctrl+E` = clear / invert / hide-tint / extract on the mask. See "WS6 layout round" in the spike results for the recovery design and the four pre-existing bugs it surfaced.
