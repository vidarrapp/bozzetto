...
| 8 | Drag brush | WS2 |
| 9 | Polish brush | WS2 shipped Twist; retired by owner call for an hPolish-style Polish (bridge PolishBrush over the Twist slot). v2 after iPad testing ("runs edges over"): a per-STROKE held plane - outlier-rejected fit (inlier vertex normals within the clip band), sticky across dabs - a Plane lock palette slider, default 0.6, after curvature feedback: 0.85 lagged a gentle curve by ~11 degrees and flattened it, while looser follows but flattens chatter more slowly; edge-stop rides on the band + normal gate, not the stickiness. Held normals are PER SYMMETRY SIDE (main/sym keyed by the picking instance): one shared normal handed the pen side's plane to the mirror pass, whose geometry then failed the normal gate against a backwards plane and barely polished, centre re-anchored per dab on the band's inliers - two-sided inside the band (polish), alt = shave-only trim, dab skipped when the band grips too little. The v1 per-dab area fit blended adjacent faces into one tilted plane, which is exactly what rounded edges off. |
...
- WS6 Layout (owner round, shipped). Four docked panels, two per edge: File over Scene on the left mirroring Render over Sculpt on the right, on a shared `SidePanel` base with a side-scoped `bozzetto:panel-open` protocol. The settings panel is renamed **Render** (tab always; header during sculpt, which also resolves a "Sculpt above Sculpt" collision from the synthetic manifest title). `Tab` hides all chrome for a minimal working screen, claimed in InputShell's capture phase so the viewer's `Tab` is untouched. Hotkey table additions: `Tab` = close open panels, then hide the interface (staged); `Esc` = show it; `Ctrl+C` / `Ctrl+I` / `Ctrl+H` / `Ctrl+E` = clear / invert / hide-tint / extract on the mask. `Ctrl` OFF the model: tap inverts the whole mask (upstream parity), drag dollies. Only upstream's drag-to-clear was displaced, since `Ctrl+C` covers it. See "WS6 layout round" in the spike results for the recovery design and the four pre-existing bugs it surfaced.
